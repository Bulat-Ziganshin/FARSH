          ...
          vmac_ctx_t *ctx)
{
    ...
    {
    #if (VMAC_TAG_LEN == 64)
        #ifdef VMAC_REQUIRE_FILL16
        nh_16(mptr,kptr,2*((remaining+15)/16),ch,cl);
        #else
        if (remaining/16) {
            nh_16(mptr,kptr,2*(remaining/16),ch,cl);
        } else {
           ch=cl=0;
        }
        if (remaining%16) {
            uint64_t ch_add,cl_add,buf[2];
            mptr += 2*(remaining/16);
            kptr += 2*(remaining/16);
            memset(buf,0,16);
            memcpy(buf,mptr,remaining%16);
            nh_16 (buf,kptr,2,ch_add,cl_add);
            ADD128(ch,cl,ch_add,cl_add);
        }
        #endif
    #else
        #ifdef VMAC_REQUIRE_FILL16
        ...
        #else
        if (remaining/16) {
            nh_16_2(mptr,kptr,2*(remaining/16),ch,cl,ch2,cl2);
        } else {
           ch=cl=ch2=cl2=0;
        }
        if (remaining%16) {
            uint64_t ch_add,cl_add,ch2_add,cl2_add,buf[2];
            mptr += 2*(remaining/16);
            kptr += 2*(remaining/16);
            memset(buf,0,16);
            memcpy(buf,mptr,remaining%16);
            nh_16_2(buf,kptr,2,ch_add,cl_add,ch2_add,cl2_add);
            ADD128(ch,cl,ch_add,cl_add);
            ADD128(ch2,cl2,ch2_add,cl2_add);
        }
        #endif
        ch2 &= m62;
        ADD128(ch2,cl2,pkh2,pkl2);
    ...
    if (remaining) {
        ...
        #else
        if (remaining/16) {
            nh_16(mptr,kptr,2*(remaining/16),rh,rl);
        } else {
           rh=rl=0;
        }
        if (remaining%16) {
            uint64_t rh_add,rl_add,buf[2];
            mptr += 2*(remaining/16);
            kptr += 2*(remaining/16);
            memset(buf,0,16);
            memcpy(buf,mptr,remaining%16);
            nh_16 (buf,kptr,2,rh_add,rl_add);
            ADD128(rh,rl,rh_add,rl_add);
        }
        #endif
    #else
        #ifdef VMAC_REQUIRE_FILL16
        ...
        #else
        if (remaining/16) {
            nh_16_2(mptr,kptr,2*(remaining/16),rh,rl,rh2,rl2);
        } else {
           rh=rl=rh2=rl2=0;
        }
        if (remaining%16) {
            uint64_t rh_add,rl_add,rh2_add,rl2_add,buf[2];
            mptr += 2*(remaining/16);
            kptr += 2*(remaining/16);
            memset(buf,0,16);
            memcpy(buf,mptr,remaining%16);
            nh_16_2(buf,kptr,2,rh_add,rl_add,rh2_add,rl2_add);
            ADD128(rh,rl,rh_add,rl_add);
            ADD128(rh2,rl2,rh2_add,rl2_add);
        }
        #endif
        rh2 &= m62;
        poly_step(ch2,cl2,pkh2,pkl2,rh2,rl2);
...
int main(void)
{
    ...
    #endif
    unsigned speed_lengths[] = {16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072};
...
*/
    ...
    for (i = 0; i < 500; i++) {
        for (j = 0; j < i; j++)
            m[j] = (unsigned char)('a'+j%3);
        res = vmac(m, i, nonce, &tagl, &ctx);
        #if (VMAC_TAG_LEN == 64)
        printf("\'abc\' * %7u: %016llX\n",
              i,res);
        #else
        printf("\'abc\' * %7u: %016llX%016llX\n",
              i,res,tagl);
