...
#define KEY_OFFSET      16  /* any key[] passed to the farsh_fast() should be aligned to this size, otherwise _mm_load_si128() in SSE2 code may fail */
#define MAX_HASHES      (1024 / (8*sizeof(UINT)))  /* hashes up to 1024 bits are supported, in 32-bit portions */
#define EXTRA_ELEMENTS  (((MAX_HASHES-1)*KEY_OFFSET)/sizeof(UINT))

ALIGN(64) static const UINT FARSH_KEYS [STRIPE_ELEMENTS + EXTRA_ELEMENTS] = {  /* STRIPE bytes of key material plus extra keys for hashes up to 1024 bits long */
...
{
    int i;  UINT *hash_ptr = (UINT*)hash;
    for (i=0; i < n; i++)
        hash_ptr[i] = farsh_keyed (data, bytes, (const char*)key + i*KEY_OFFSET, seed);
}
...
void farsh_n (const void *data, size_t bytes, int k, int n, ULONG seed, void *hash)
{
    ...
    farsh_keyed_n (data, bytes, (const char*)FARSH_KEYS + k*KEY_OFFSET, n, seed, hash);
}
